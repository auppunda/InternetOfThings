...
void connect_server() {
	...
	ERR_load_crypto_strings();
  	...
  	{
  		fprintf(stderr, "ssl_library wont init\n");
  		exit(2);
  	}
    ...
    if(ctx == NULL) {
    	fprintf(stderr, "Unable to create a SSL structure.\n");
    	exit(2);
    }
    ...
    if(ssl == NULL) {
    	fprintf(stderr, "SSL handshake not completed\n");
    	exit(2);
    }
    ...
    if(SSL_set_fd(ssl, sockfd) == 0) {
    	fprintf(stderr, "couldn't bind tls to socket\n");
    	exit(2);
    }
...
void startup() {
	...
	while(!off) {
		...
		if(std_in.revents & POLLIN) {
			//printf("fuck me");
			sizeread = SSL_read(ssl, command_buffer+offset, 256-offset-1);
...
int main(int argc, char **argv) {
	...
	if(port == 0) {
		fprintf(stderr, "Invalid Port : %s", strerror(errno));
		exit(1);
	}
	connect_server();
